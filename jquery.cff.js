...
(function($, cff){
    ...
    $.cff = function(opts){
        ...
        if(opts == undefined){
            opts = $.fn.cff.defaults;
        }

        this.options = opts;

        this.date_milliseconds = 0;
        this.date_seconds = 0;
        this.date_minutes = 0;
        this.date_hours = 0;

        this.angle_seconds = 0;
        this.angle_minutes = 0;
        this.angle_hours = 0;

        this.old_angle_seconds = 0;
        this.old_angle_minutes = 0;
        this.first_load = true;

    };

    // Class functions
    $.extend($.cff.prototype, {
        ...
        init: function(element){
            ...
            this.seconds = $div.clone().addClass('CffSeconds');
            this.minutes = $div.clone().addClass('CffMinutes');
            this.hours = $div.clone().addClass('CffHours');

            if(this.options.logo){
                this.logo = $div.clone().addClass('CffLogo');
                this.container.append(this.logo);
            }

            this.container.addClass('CffClock')
                .append(this.hours)
                .append(this.minutes)
                .append(this.seconds);
            ...
            setInterval(function(){ $this.tick() } , 200);
        ...
        tick: function(){
            ...
            this.date_milliseconds = d.getMilliseconds();
            this.date_seconds = d.getSeconds();
            this.date_minutes = d.getMinutes();
            this.date_hours = d.getHours();

            if(this.date_hours >= 12){
                this.date_hours-= 12;
            }

            this.angle_seconds = (this.date_seconds + this.date_milliseconds / 1000) * (6 + this.options.delay);

            if(this.angle_seconds >= 360){
                if(this.date_seconds == 0){
                    this.angle_seconds = (this.date_seconds + this.date_milliseconds / 1000) * (6 + this.options.delay);
                } else {
                    this.angle_seconds = 0;
                }
            ...
            this.angle_minutes = this.date_minutes * 6;
            this.angle_hours = (this.date_hours + this.angle_minutes / 360) * 30;

            if(this.old_angle_seconds > this.angle_seconds && this.angle_seconds == 0 || this.old_angle_seconds == 360){
                if(this.old_angle_seconds < 360){
                    this.angle_seconds = 360;
                } else {
                    this.seconds.stopRotate();
                    this.seconds.rotate(0);
                    this.angle_seconds = 0;
                }
            }

            if(this.old_angle_minutes > this.angle_minutes && this.angle_minutes == 0 || this.old_angle_minutes == 360){
                if(this.old_angle_minutes < 360){
                    this.angle_minutes = 360;
                } else {
                    this.minutes.stopRotate();
                    this.minutes.rotate(0);
                    this.angle_minutes = 0;
                }
            }

            if(this.first_load){
                this.seconds.rotate(this.angle_seconds);
                this.minutes.rotate(this.angle_minutes);
                this.hours.rotate(this.angle_hours);
                this.first_load = false
            } else {
                this.seconds.rotate({ animateTo: this.angle_seconds });
                this.minutes.rotate({ animateTo: this.angle_minutes, duration: 200 });
                this.hours.rotate(this.angle_hours);
            }

            this.old_angle_seconds = this.angle_seconds;
            this.old_angle_minutes = this.angle_minutes;

        }

    });

    $.fn.cff = function(options){
        ...
        var opt = $.extend({}, $.fn.cff.defaults, options);
